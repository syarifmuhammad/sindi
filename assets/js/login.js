! function(global, factory) {
    ...
    jQuery.fn = jQuery.prototype = {
        ...
        for ("boolean" == typeof target && (deep = target, target = arguments[i] || {}, i++), "object" == typeof target || isFunction(target) || (target = {}), i === length && (target = this, i--); i < length; i++)
            if (null != (options = arguments[i]))
                for (name in options) src = target[name], target !== (copy = options[name]) && (deep && copy && (jQuery.isPlainObject(copy) || (copyIsArray = Array.isArray(copy))) ? (clone = copyIsArray ? (copyIsArray = !1, src && Array.isArray(src) ? src : []) : src && jQuery.isPlainObject(src) ? src : {}, target[name] = jQuery.extend(deep, clone, copy)) : void 0 !== copy && (target[name] = copy));
        return target
    }, jQuery.extend({
    ...
    jQuery.Animation = jQuery.extend(Animation, {
            ...
            tweener: function(props, callback) {
                for (var prop, index = 0, length = (props = isFunction(props) ? (callback = props, ["*"]) : props.match(rnothtmlwhite)).length; index < length; index++) prop = props[index], Animation.tweeners[prop] = Animation.tweeners[prop] || [], Animation.tweeners[prop].unshift(callback)
            },
...
function(define) {
    define(["jquery"], function($) {
        return function() {
            var $container, listener, previousToast, toastId = 0,
                toastType_error = "error",
                toastType_info = "info",
                toastType_success = "success",
                toastType_warning = "warning",
                toastr = {
            ...
            function getContainer(options, create) {
                return options || (options = getOptions()), ($container = $("#" + options.containerId)).length || create && ($container = function(options) {
                    return ($container = $("<div/>").attr("id", options.containerId).addClass(options.positionClass).attr("aria-live", "polite").attr("role", "alert")).appendTo($(options.target)), $container
                }(options)), $container
            }
            ...
            function notify(map) {
                    ...
                    return map.iconClass && $toastElement.addClass(options.toastClass).addClass(iconClass), map.title && ($titleElement.append(map.title).addClass(options.titleClass), $toastElement.append($titleElement)), map.message && ($messageElement.append(map.message).addClass(options.messageClass), $toastElement.append($messageElement)), options.closeButton && ($closeElement.addClass("toast-close-button").attr("role", "button"), $toastElement.prepend($closeElement)), options.progressBar && ($progressElement.addClass("toast-progress"), $toastElement.prepend($progressElement)), options.newestOnTop ? $container.prepend($toastElement) : $container.append($toastElement), $toastElement.hide(), $toastElement[options.showMethod]({
                        ...
                        function() {
                            ...
                            options.closeButton && $closeElement && $closeElement.click(function(event) {
                                event.stopPropagation ? event.stopPropagation() : void 0 !== event.cancelBubble && !0 !== event.cancelBubble && (event.cancelBubble = !0), hideToast(!0)
                            });
                ...
                function hideToast(override) {
                    if (!$(":focus", $toastElement).length || override) return clearTimeout(progressBar.intervalId), $toastElement[options.hideMethod]({
                        duration: options.hideDuration,
                        easing: options.hideEasing,
                        complete: function() {
                            removeToast($toastElement), options.onHidden && "hidden" !== response.state && options.onHidden(), response.state = "hidden", response.endTime = new Date, publish(response)
                        }
                    })
                }
...
function subscriber() {
    ...
    _H.Loading(!0), $.post(_BASE_URL + "public/subscriber/save", values, function(response) {
        var res = _H.StrToObject(response);
        _H.Notify(res.status, res.message), $("#subscriber").val(""), _H.Loading(!1)
    })
}
...
function polling() {
    ...
    answer_id ? $.post(_BASE_URL + "public/pollings/save", values, function(response) {
        var res = _H.StrToObject(response);
        _H.Notify(res.status, res.message), _H.Loading(!1)
    }) : (_H.Notify("info", "Anda belum memilih jawaban."), _H.Loading(!1))
}
...
function contact_us() {
    ...
    _RECAPTCHA_STATUS && (values.recaptcha = $('textarea[name="g-recaptcha-response"]').val()), $.post(_BASE_URL + "hubungi-kami/save", values, function(response) {
        _H.Loading(!1);
        var res = _H.StrToObject(response);
        _H.Notify(res.status, res.message), "success" == res.status && $('input[type="text"], textarea').val(""), _RECAPTCHA_STATUS && grecaptcha.reset()
    })
}
...
function post_comment() {
    ...
    _RECAPTCHA_STATUS && (values.recaptcha = $('textarea[name="g-recaptcha-response"]').val()), $.post(_BASE_URL + "public/post_comments", values, function(response) {
        _H.Loading(!1);
        var res = _H.StrToObject(response);
        _H.Notify(res.status, res.message), "success" == res.status && $('input[type="text"], textarea').val(""), _RECAPTCHA_STATUS && grecaptcha.reset()
    })
...
function photo_preview(id) {
    ...
    $.post(_BASE_URL + "public/gallery_photos/preview", values, function(response) {
        _H.Loading(!1), 0 < response.count ? $.magnificPopup.open({
...
function save_registration_form() {
    ...
    fill_data.append("is_transfer", $("#is_transfer").val()), fill_data.append("admission_type_id", $("#admission_type_id").val()), fill_data.append("full_name", $("#full_name").val()), fill_data.append("gender", $("#gender").val()), fill_data.append("birth_date", $("#birth_date").val()), fill_data.append("district", $("#district").val()), $("#photo").length && fill_data.append("photo", $('input[name="photo"]')[0].files[0]), $("#first_choice_id").length && fill_data.append("first_choice_id", $("#first_choice_id").val()), $("#second_choice_id").length && fill_data.append("second_choice_id", $("#second_choice_id").val()), $("#photo").length && fill_data.append("photo", $("#photo").val()), $("#nisn").length && fill_data.append("nisn", $("#nisn").val()), $("#nik").length && fill_data.append("nik", $("#nik").val()), $("#prev_exam_number").length && fill_data.append("prev_exam_number", $("#prev_exam_number").val()), $("#prev_school_name").length && fill_data.append("prev_school_name", $("#prev_school_name").val()), $("#prev_school_address").length && fill_data.append("prev_school_address", $("#prev_school_address").val()), $("#achievement").length && fill_data.append("achievement", $("#achievement").val()), $("#paud").length && fill_data.append("paud", $("#paud").val()), $("#tk").length && fill_data.append("tk", $("#tk").val()), $("#skhun").length && fill_data.append("skhun", $("#skhun").val()), $("#prev_diploma_number").length && fill_data.append("prev_diploma_number", $("#prev_diploma_number").val()), $("#hobby").length && fill_data.append("hobby", $("#hobby").val()), $("#ambition").length && fill_data.append("ambition", $("#ambition").val()), $("#birth_place").length && fill_data.append("birth_place", $("#birth_place").val()), $("#religion_id").length && fill_data.append("religion_id", $("#religion_id").val()), $("#special_need_id").length && fill_data.append("special_need_id", $("#special_need_id").val()), $("#street_address").length && fill_data.append("street_address", $("#street_address").val()), $("#rt").length && fill_data.append("rt", $("#rt").val()), $("#rw").length && fill_data.append("rw", $("#rw").val()), $("#sub_village").length && fill_data.append("sub_village", $("#sub_village").val()), $("#village").length && fill_data.append("village", $("#village").val()), $("#sub_district").length && fill_data.append("sub_district", $("#sub_district").val()), $("#postal_code").length && fill_data.append("postal_code", $("#postal_code").val()), $("#residence_id").length && fill_data.append("residence_id", $("#residence_id").val()), $("#transportation_id").length && fill_data.append("transportation_id", $("#transportation_id").val()), $("#phone").length && fill_data.append("phone", $("#phone").val()), $("#mobile_phone").length && fill_data.append("mobile_phone", $("#mobile_phone").val()), $("#email").length && fill_data.append("email", $("#email").val()), $("#sktm").length && fill_data.append("sktm", $("#sktm").val()), $("#kks").length && fill_data.append("kks", $("#kks").val()), $("#kps").length && fill_data.append("kps", $("#kps").val()), $("#kip").length && fill_data.append("kip", $("#kip").val()), $("#kis").length && fill_data.append("kis", $("#kis").val()), $("#citizenship").length && fill_data.append("citizenship", $("#citizenship").val()), $("#country").length && fill_data.append("country", $("#country").val()), $("#father_name").length && fill_data.append("father_name", $("#father_name").val()), $("#father_birth_year").length && fill_data.append("father_birth_year", $("#father_birth_year").val()), $("#father_education_id").length && fill_data.append("father_education_id", $("#father_education_id").val()), $("#father_employment_id").length && fill_data.append("father_employment_id", $("#father_employment_id").val()), $("#father_monthly_income_id").length && fill_data.append("father_monthly_income_id", $("#father_monthly_income_id").val()), $("#father_special_need_id").length && fill_data.append("father_special_need_id", $("#father_special_need_id").val()), $("#mother_name").length && fill_data.append("mother_name", $("#mother_name").val()), $("#mother_birth_year").length && fill_data.append("mother_birth_year", $("#mother_birth_year").val()), $("#mother_education_id").length && fill_data.append("mother_education_id", $("#mother_education_id").val()), $("#mother_employment_id").length && fill_data.append("mother_employment_id", $("#mother_employment_id").val()), $("#mother_monthly_income_id").length && fill_data.append("mother_monthly_income_id", $("#mother_monthly_income_id").val()), $("#mother_special_need_id").length && fill_data.append("mother_special_need_id", $("#mother_special_need_id").val()), $("#guardian_name").length && fill_data.append("guardian_name", $("#guardian_name").val()), $("#guardian_birth_year").length && fill_data.append("guardian_birth_year", $("#guardian_birth_year").val()), $("#guardian_education_id").length && fill_data.append("guardian_education_id", $("#guardian_education_id").val()), $("#guardian_employment_id").length && fill_data.append("guardian_employment_id", $("#guardian_employment_id").val()), $("#guardian_monthly_income_id").length && fill_data.append("guardian_monthly_income_id", $("#guardian_monthly_income_id").val()), $("#mileage").length && fill_data.append("mileage", $("#mileage").val()), $("#traveling_time").length && fill_data.append("traveling_time", $("#traveling_time").val()), $("#height").length && fill_data.append("height", $("#height").val()), $("#weight").length && fill_data.append("weight", $("#weight").val()), $("#sibling_number").length && fill_data.append("sibling_number", $("#sibling_number").val()), fill_data.append("declaration", $("#declaration").prop("checked")), _RECAPTCHA_STATUS && fill_data.append("recaptcha", $('textarea[name="g-recaptcha-response"]').val()), $.ajax({
        ...
        success: function(response) {
            _H.Loading(!1);
            var res = _H.StrToObject(response);
            _H.Notify(res.status, res.message), "success" == res.status && window.open(_BASE_URL + "media_library/students/" + res.file_name, "_self"), _RECAPTCHA_STATUS && grecaptcha.reset()
        }
    })
}
...
function print_admission_form() {
    ...
    _RECAPTCHA_STATUS && (values.recaptcha = $('textarea[name="g-recaptcha-response"]').val()), $.post(_BASE_URL + "public/print_admission_form/process", values, function(response) {
        var res = _H.StrToObject(response);
        "success" == res.status ? window.open(_BASE_URL + "media_library/students/" + res.file_name, "_self") : (_H.Notify(res.status, res.message), _H.Loading(!1), _RECAPTCHA_STATUS && grecaptcha.reset())
    })
}
...
function selection_results() {
    ...
    _RECAPTCHA_STATUS && (values.recaptcha = $('textarea[name="g-recaptcha-response"]').val()), _H.Loading(!0), $.post(_BASE_URL + "public/admission_selection_results/selection_results", values, function(response) {
        _H.Loading(!1);
        var res = _H.StrToObject(response);
        _H.Notify(res.status, res.message), _RECAPTCHA_STATUS && grecaptcha.reset()
    })
}
_H.NestedList = function(menus) {
    var str = "";
    for (var z in menus) {
        var menu = menus[z];
        str += '<li class="dd-item" data-id="' + menu.id + '">', str += '<div class="dd-handle">' + menu.menu_title.toUpperCase() + "</div>";
        var sub_menus = _H.NestedList(menu.child);
        str += sub_menus ? '<ol class="dd-list">' + sub_menus + "</ol>" : "</li>"
    }
    return str
}, _H.StrToObject = function(str) {
    if ("object" == typeof str) return str;
    if ("string" != typeof str) throw console.log(str), "cannot parse non-json-string";
    ...
    return void 0 === str ? month : month[str]
}, _H.ToIndonesianDate = function(str) {
    if (void 0 !== str) {
        var explode = str.split("-"),
            year = explode[0],
            month = explode[1];
        return explode[2] + " " + _H.Month(month) + " " + year
    }
}, _H.FormatBytes = function(bytes, decimals) {
    if (0 == bytes) return "0 Byte";
    var dm = decimals + 1 || 3,
        i = Math.floor(Math.log(bytes) / Math.log(1e3));
    return (bytes / Math.pow(1e3, i)).toPrecision(dm) + " " + ["Bytes", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"][i]
}, _H.Message = function(message) {
    var msg = "";
    switch (message) {
        case "created":
        ...
        case "not_created":
            ...
            break;
        case "updated":
            msg = "Data Anda berhasil diperbaharui !";
            break;
        case "not_updated":
        ...
        case "not_deleted":
            ...
            break;
        case "restored":
            msg = "Data Anda berhasil dikembalikan !";
            break;
        case "not_restored":
        ...
        case "forbidden":
            ...
            break;
        case "extracted":
            msg = "Tema berhasil diekstrak";
            break;
        case "not_extracted":
            msg = "Tema gagal diekstrak";
            break;
        default:
            msg = message
    }
    ...
    isTrue ? $("body").addClass("loading") : $("body").removeClass("loading")
}, String.prototype.ucwords = function() {
    return str = this.toLowerCase(), str.replace(/(^([a-zA-Z\p{M}]))|([ -][a-zA-Z\p{M}])/g, function(s) {
        return s.toUpperCase()
    })
}, _H.MapToAttr = function(map) {
    if (!map) return "";
    if ("string" == typeof map) return map;
    var str = " ";
    for (var z in map) {
        var v = map[z];
        null != v && ("trigger" != z ? str += z + '="' + v + '" ' : map.disabled || (str += _H.triggerCompleteChangeAttr(map.trigger)))
    }
    return str
}, _H.triggerCompleteChangeAttr = function(handler) {
    if (!handler) return "";
    var prop = "";
    return prop += ' onblur="' + handler + '(event,this)" onkeyup="' + handler + '(event,this)" onkeydown="' + handler + '(event,this)" onchange="' + handler + '(event,this)" '
}, _H.OpenNewTab = function(url) {
    console.log(url), url ? window.open(url, url) : _H.Notify("info", url)
    ...
    var time = _H.RealtimeDate();
    $("a.current-time").html(time)
}, 1e3), _H.RealtimeDate = function() {
    var a = new Date,
        b = [];
    b[0] = "Januari", b[1] = "Februari", b[2] = "Maret", b[3] = "April", b[4] = "Mei", b[5] = "Juni", b[6] = "Juli", b[7] = "Agustus", b[8] = "September", b[9] = "Oktober", b[10] = "Nopember", b[11] = "Desember";
    ...
    var currentDay = c[a.getDay()],
        ...
        e = a.getMinutes(),
        f = a.getSeconds(),
        time = (d = (d < 10 ? "0" : "") + d) + ":" + (e = (e < 10 ? "0" : "") + e) + ":" + (f = (f < 10 ? "0" : "") + f);
    return (currentDay + " " + currentDate + " " + currentMonth + " " + currentYear).toUpperCase() + " &sdot; " + time
}, toastr.options = {
    closeButton: !0,
    debug: !1,
    newestOnTop: !1,
...
function xisNumberx(e){e=e||window.event;var i=e.which?e.which:e.keyCode;return!(i>31&&(i<48||i>57)||13==i)};