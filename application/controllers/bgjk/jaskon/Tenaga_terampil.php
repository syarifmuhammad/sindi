...
class Tenaga_terampil extends Admin_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->load->model('bgjk/jaskon/m_tenaga_terampil');
		$this->pk = M_tenaga_terampil::$pk;
		$this->table = M_tenaga_terampil::$table;
	}
	...
	public function index() {
		$this->vars['title'] = 'Data Tenaga Terampil';
		$this->vars['bgjk'] =  $this->vars['jaskon'] = $this->vars['tenaga_terampil'] = true;
		$this->vars['content'] = 'bgjk/jaskon/tenaga_terampil';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_tenaga_terampil->get_where($keyword, $limit, $offset);
			$total_rows = $this->m_tenaga_terampil->total_rows($keyword);
	...
	public function import_excel_form() {
		$this->vars['title'] = 'Import Data Tenaga Terampil';
		$this->vars['save_action'] = 'bgjk/jaskon/tenaga_terampil/save_ecxel';
	...
	public function save_ecxel() {
		if ($this->input->is_ajax_request()) {
			...
			foreach($rows as $row) {
				$exp = explode("\t", $row);
				if (count($exp) != 10) continue;
				$fill_data = [];
				$fill_data['tahun'] 				= $this->session->tahun;
				$fill_data['nama'] 					= trim($exp[0]);
				$fill_data['nik'] 					= trim($exp[1]);
				$fill_data['jk'] 					= trim($exp[2]);
				$fill_data['alamat'] 				= trim($exp[3]);
				$fill_data['no_skt'] 				= trim($exp[4]);
				$fill_data['tgl_skt'] 				= trim($exp[5]);
				$fill_data['jenis_keterampilan']	= trim($exp[6]);
				$fill_data['penerbit_skt'] 			= trim($exp[7]);
				$fill_data['telp'] 					= trim($exp[8]);
				$fill_data['email'] 				= trim($exp[9]);
				$fill_data['created_at'] 			= date('Y-m-d H:i:s');
				$fill_data['created_by'] 			= $this->session->user_id;
				$query = $this->model->is_exists_array(['tahun' => $fill_data['tahun'], 'nik' => $fill_data['nik']], $this->table);
				if (!$query) {
